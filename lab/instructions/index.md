...
1. Before you continue with the lab, confirm that the deployments succeeded by running the following command:
    ...
    !IMAGE[deployment-status.png](instructions275721/deployment-status.png)
...
Here's an overview of the cloud resources you just deployed.
...
1. Run the job with the following command:
    ...
    It'll take a few moments for the job to run. When the job exits, it might some warnings; it's safe to ignore them.
<!--
...
1. In the chat UI, enter `between a 12" round deep dish pizza that is 1.5" thick and an 18" round thin crust pizza that is half inch thick, which one is larger by volume?`
    ...
    !IMAGE[agent-output.png](instructions275721/agent-output.png)
    ...
    * `I just saw a lightning strike in Lake Michigan, and I heard the thunder 22 seconds later. How far away is the storm?`
...
1. In VS Code, open your project's *src/chat_app.py*.
...
1. The indexer job runs every hour to add documents from the file share to the AI Search index. To kick off a job execution now, run the following command to start an execution and save its name to a variable:
